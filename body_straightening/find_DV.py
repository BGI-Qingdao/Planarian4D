...
for i in 'WT','0hpa1','0hpa2','3dpa1','3dpa2','5dpa1','5dpa2','7dpa1','7dpa2','10dpa1','10dpa2','12hpa1','12hpa2','14dpa1','14dpa2','36hpa1','36hpa2':
    ...
    test = io.imread(f'input/{i}_neuralmesh.tif')
    test = test.astype('uint8')
    test = RGB2wh(test)
    raw = io.imread(f'input/{i}_neuralmesh.tif')
    ...
    constrain = pd.read_csv('constrain.txt', sep="\t")
    ...
    #io.imsave(f'C:/Users/jizhi1/Desktop/correct/AMP_line/colour/{i}_colour.tif', colour2)
    colour = colour.astype('uint8')
    io.imsave(f'output/{i}_line_image.tif', colour)
...
p.to_csv('line_parameter.txt',sep='\t',header=True,index=False)
